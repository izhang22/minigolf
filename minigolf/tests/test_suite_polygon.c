...
void test_square_translate() {
    ...
    assert(vec_equal(*get_vector_from_polygon(sq, 0), (vector_t){3, 4}));
    assert(vec_equal(*get_vector_from_polygon(sq, 1), (vector_t){1, 4}));
    assert(vec_equal(*get_vector_from_polygon(sq, 2), (vector_t){1, 2}));
    assert(vec_equal(*get_vector_from_polygon(sq, 3), (vector_t){3, 2}));
    assert(isclose(polygon_area(sq), 4));
...
void test_square_rotate() {
    ...
    assert(vec_isclose(*get_vector_from_polygon(sq, 0), (vector_t){0, sqrt(2)}));
    assert(vec_isclose(*get_vector_from_polygon(sq, 1), (vector_t){-sqrt(2), 0}));
    assert(vec_isclose(*get_vector_from_polygon(sq, 2), (vector_t){0, -sqrt(2)}));
    assert(vec_isclose(*get_vector_from_polygon(sq, 3), (vector_t){sqrt(2), 0}));
    assert(isclose(polygon_area(sq), 4));
    assert(vec_isclose(polygon_centroid(sq), VEC_ZERO));
    list_free(sq);
}
...
void test_triangle_translate() {
    ...
    assert(vec_equal(*get_vector_from_polygon(tri, 0), (vector_t){-4, -3}));
    assert(vec_equal(*get_vector_from_polygon(tri, 1), (vector_t){0, -3}));
    assert(vec_equal(*get_vector_from_polygon(tri, 2), (vector_t){0, 0}));
    assert(isclose(polygon_area(tri), 6));
...
void test_triangle_rotate() {
    ...
    assert(vec_isclose(*get_vector_from_polygon(tri, 0), (vector_t){-1, 3}));
    assert(vec_isclose(*get_vector_from_polygon(tri, 1), (vector_t){2.2, 0.6}));
    assert(vec_isclose(*get_vector_from_polygon(tri, 2), (vector_t){4, 3}));
    assert(isclose(polygon_area(tri), 6));
...
void test_circ_translate() {
    ...
    for (size_t i = 0; i < CIRC_NPOINTS; i++) {
        double angle = 2 * M_PI * i / CIRC_NPOINTS;
        assert(vec_isclose(*get_vector_from_polygon(c, i),
...
void test_weird_translate() {
    ...
    assert(vec_isclose(*get_vector_from_polygon(w, 0), (vector_t){-10, -20}));
    assert(vec_isclose(*get_vector_from_polygon(w, 1), (vector_t){-6, -19}));
    assert(vec_isclose(*get_vector_from_polygon(w, 2), (vector_t){-12, -19}));
    assert(vec_isclose(*get_vector_from_polygon(w, 3), (vector_t){-15, -15}));
    assert(vec_isclose(*get_vector_from_polygon(w, 4), (vector_t){-11, -28}));
    assert(isclose(polygon_area(w), 23));
...
void test_weird_rotate() {
    ...
    assert(vec_isclose(*get_vector_from_polygon(w, 0), (vector_t){2, 2}));
    assert(vec_isclose(*get_vector_from_polygon(w, 1), (vector_t){1, 6}));
    assert(vec_isclose(*get_vector_from_polygon(w, 2), (vector_t){1, 0}));
    assert(vec_isclose(*get_vector_from_polygon(w, 3), (vector_t){-3, -3}));
    assert(vec_isclose(*get_vector_from_polygon(w, 4), (vector_t){10, 1}));
    assert(isclose(polygon_area(w), 23));
